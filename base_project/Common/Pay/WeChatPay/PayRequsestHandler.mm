...
-(NSString *)sendPrepay:(NSMutableDictionary *)prePayParams
{
    ...
    if ( [return_code isEqualToString:@"SUCCESS"] )
    {
        ...
        if( [sign isEqualToString:send_sign]){
            ...
            {
                UIAlertView *av=[[UIAlertView alloc] initWithTitle:@"支付失败" message:[resParams objectForKey:@"err_code_des"] delegate:nil cancelButtonTitle:@"确定" otherButtonTitles:nil, nil];
                [av show];
            }
        }else{
            last_errcode = 1;
