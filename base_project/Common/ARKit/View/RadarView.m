...
- (void) updatePoints:(ARGeoCoordinate *)centerCoord {
	...
	for (ARGeoCoordinate *coord in points) {
        ...
        //。。。待修改 半径算法
		pt.x = dotsView.center.x + cos(coordAzimuth) * coord.radialDistance * (self.frame.size.width-20) / (2 * farthest);
		pt.y = dotsView.center.y + sin(coordAzimuth) * coord.radialDistance * (self.frame.size.height-20) / (2 * farthest);
