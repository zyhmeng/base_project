...
- (id)initWithFrame:(CGRect)frame
{
    ...
    if (self) {
        ...
        if (_laberColor==nil) {
            _laberColor=[UIColor blackColor];
        }else
            [self setTitleColor:_laberColor forState:UIControlStateNormal];
    }
    return self;
}
...
- (CGSize):(CGSize)size
{
    ...
    }else if (self.buttonOrientation==ALOCenteredButtonOrientationLeftToRight)
    {
        return CGSizeMake(labelSize.width + imageSize.width + self.imageLabelSpacing,
                          MAX(labelSize.height, imageSize.height));
    }
...
- (void)layoutSubviews
{
    ...
    if (self.buttonOrientation == ALOCenteredButtonOrientationRightToLeft) {
        ...
        else if (self.buttonOrientation==ALOCenteredButtonOrientationLeftToRight)
        {
            ...
            CGFloat buttonWidth = CGRectGetWidth(self.bounds);
            // reposition label
            CGRect labelFrame = self.imageView.frame;
            ...
            // reposition icon
            CGRect imageFrame = self.titleLabel.frame;
            imageFrame.origin = CGPointMake(CGRectGetMaxX(labelFrame) + self.imageLabelSpacing,
                                            truncf((self.bounds.size.height - imageFrame.size.height) / 2));
            
            self.titleLabel.frame = imageFrame;
            self.imageView.frame = labelFrame;
        }
    else {
